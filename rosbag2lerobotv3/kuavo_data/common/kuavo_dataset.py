...
class KuavoRosbagReader:
    ...
    def list_bag_files(self, bag_dir: str):
        ...
        bag_files = [f for f in bag_files if not f.endswith('.c.bag')]
        bag_files.sort()
        return bag_files
    ...
    def get_topics(self, bag_file: str):
        """返回bag文件所有话题名列表"""
        bag = rosbag.Bag(bag_file)
        return list(bag.get_type_and_topic_info().topics.keys())

    def get_frame_rate(self, bag_file: str, topic: str = None):
        """检测指定话题的帧率（默认主相机）"""
        bag = rosbag.Bag(bag_file)
        if topic is None:
            # 默认检测第一个主相机
            for cam in DEFAULT_CAMERA_NAMES:
                topic = self._topic_process_map.get(cam, {}).get("topic", None)
                if topic:
                    break
        times = []
        for _, msg, t in bag.read_messages(topics=topic):
